...
async function randomClicks(page) {
    ...
    return await handleAsync(randomElement.click({
        delay: 100,
        count: 3,
    }));
}
...
export async function main(questionQuery, withProxy) {
    ...
    const anchor = await page.$('#search .MjjYud .yuRUbf a[href^="https://stackoverflow.com/"]');
    if (!anchor)
        return {
            type: 1,
    ...
    const [answers] = await handleAsync(answersBlock.$$eval('div.answer[id^="answer-"] .answercell.post-layout--right > .s-prose.js-post-body', (elementList) => {
